...
impl BitBoard {
    ...
    pub fn from_sq(sq: usize) -> BitBoard {
        debug_assert!(sq < 64);
        BitBoard(1 << sq)
    }

    /// Create a bitboard from a vector of square indices
    pub fn from_sq_vec(sqs: Vec<usize>) -> BitBoard {
        let mut result = 0;
    ...
    pub fn hq_rook_attacks(&self, occ: BitBoard) -> BitBoard {
        self.hq_file_attacks(occ) | self.hq_rank_attacks(occ)
    }

    /// Uses hyperbola quintessence to find the attack squares of a single
    /// bishop, taking into account the occupancy of the current board
    pub fn hq_bishop_attacks(&self, occ: BitBoard) -> BitBoard {
        self.hq_diag_attacks(occ) | self.hq_adiag_attacks(occ)
    }
    ...
    pub fn between_mask(&self, other: BitBoard) -> BitBoard {
        ...
        panic!("no axis between {} and {}", self.to_sq(), other.to_sq())
    }
...
mod tests {
    ...
    fn test_squares_to_bitboard() {
        ...
        let bitboard = BitBoard::from_sq_vec(squares);
        assert_eq!(bitboard, rank::RANK_1);
    }

    #[test]
    fn test_get_ls1b() {
        let bb = BitBoard::from_sq_vec(vec![19, 30]);
        let lsb = bb.get_ls1b();
        assert_eq!(BitBoard::from_sq(19), lsb);
    }

    #[test]
    fn test_get_ilsb() {
        let bitboard = BitBoard::from_sq_vec(vec![41, 55]);
    ...
    #[test_case(BitBoard::hq_rank_attacks, vec![17, 19, 23, 35], 19, vec![17, 18, 20, 21, 22, 23];"RANK")]
    #[test_case(BitBoard::hq_file_attacks, vec![20, 44, 18], 20, vec![4, 12, 28, 36, 44];"FILE")]
    #[test_case(BitBoard::hq_diag_attacks, vec![27, 54, 18], 27, vec![18, 36, 45, 54];"DIAG")]
    #[test_case(BitBoard::hq_adiag_attacks, vec![6, 13, 34, 41, 43], 34, vec![41, 27, 20, 13];"ADIAG")]
    fn test_hyp_quint(
        func: fn(&BitBoard, BitBoard) -> BitBoard,
        occ: Vec<usize>,
        slider: usize,
        expected: Vec<usize>,
    ) {
        let occ = BitBoard::from_sq_vec(occ);
        let slider = BitBoard::from_sq(slider);
        let result = func(&slider, occ);
        let expected = BitBoard::from_sq_vec(expected);
