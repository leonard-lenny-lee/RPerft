...
fn find_double_pushes(
    ...
    let sgl_push = pos.pawn_sgl_push(
        our_pieces.pawn & pos.pawn_start_rank()) & pos.data.free;
...
fn find_pawn_captures(
    ...
    | (pos.pawn_left_capture(pinned_pawns) & target_filter & pos.pawn_left_capture_pin_mask(our_pieces.king));
    ...
    | (pos.pawn_right_capture(pinned_pawns) & target_filter & pos.pawn_right_capture_pin_mask(our_pieces.king));
...
fn find_sliding_moves(
    ...
    for src in rook_movers & pinned_pieces {
        let targets = src.lookup_rook_attacks(pos.data.occ)
    ...
    for src in bishop_movers & pinned_pieces {
        let targets = src.lookup_bishop_attacks(pos.data.occ) 
            & filter & our_pieces.king.common_axis(src);
        find_quiet_moves_and_captures(pos, move_list, targets, src)
    }
...
fn find_quiet_moves_and_captures(
    pos: &Position, move_list: &mut MoveList, targets: BB, src: BB
) {
